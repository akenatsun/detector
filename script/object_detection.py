...
class ObjectDetection(object):
    ...
    def main(self):
        person = cv2.dnn.readNet("weight/yolov3-tiny.weights", "cfg/yolov3-tiny.cfg")
        classes = []
        with open("cfg/yolov3-tiny.cfg", "r") as f:
            classes = [line.strip() for line in f.readlines()] 

        output_layers = [layer_name for layer_name in person.getUnconnectedOutLayersNames()]
        ...
        while not rospy.is_shutdown():
            ...
            person.setInput(blob)
            outputs = person.forward(output_layers)
            boxes = []
            confs = []
            class_ids = []
